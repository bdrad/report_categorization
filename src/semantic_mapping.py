        ...
        if self.negex_range == 1:
            for labeled_report in labeled_reports:
                ...
                for sentence in labeled_report[0]:
                    new_report.append(sentence.replace(
                        "NEGEX ", "NEGEX_").replace("NEGEX_NEGEX_", "NEGEX_"))
                    if self.remove:
                        new_report = " ".join(
                            filter(lambda t: "NEGEX_" not in t, new_report.split(" ")))
            ...
            for labeled_report in labeled_reports:
                ...
                for sentence in report:
                    ...
                    negex_indices = [i for i, t in enumerate(
                        tokenized) if t == "NEGEX"]
                    to_negate = []
                    for offset in range(1, self.negex_range + 1):
                        to_negate += [i + offset for i in negex_indices]
                    to_negate = [
                        tn for tn in to_negate if not tn in negex_indices]
                    to_negate = [tn for tn in to_negate if tn <
                                 len(tokenized) and tn >= 0]
                    ...
                    for i in to_negate:
                        if tokenized[i] == "EXT":
                            ...
                            to_negate = to_negate + \
                                [r + i for r in range(1, self.negex_range + 1)]
                    to_negate = [tn for tn in to_negate if not (
                        tn in negex_indices or tn in EXT_indices)]
                    to_negate = [tn for tn in to_negate if tn <
                                 len(tokenized) and tn >= 0]
                    ...
                    if self.remove:
                        tokenized = filter(
                            lambda t: "NEGEX_" not in t, tokenized)
                    clean_tokenized = [t for i, t in enumerate(
                        tokenized) if not i in negex_indices]

                    new_sentences.append(
                        " ".join(clean_tokenized).replace("NEGEX_NEGEX_", "NEGEX_"))
...
class PhraseDetector(TransformerMixin):
    def transform(self, labeled_reports, *_):
        ...
        for labeled_report in labeled_reports:
            report = labeled_report[0]
            new_report = [" ".join(bigram[sentence.split(" ")])
                          for sentence in report]
            result.append((new_report, labeled_report[1]))
        return result
    ...
    def transform(self, labeled_reports, *_):
        ...
        for report in labeled_reports:
            ...
            for sentence in report[0]:
                ...
                filtered_sentence = [w for w in words if (
                    not w in to_remove) and (not w.isdigit())]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Preprocess a corpus and output it to a file')
    ...
    pipeline = make_pipeline(DateTimeMapper, ExtenderPreserver, ReplacementMapper,
